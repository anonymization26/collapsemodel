...
def collapse_greedy(feats, rc, all_ds, k):
    ...
    snorm_cache = {}
    for d in all_ds:
        _, s, Vt = np.linalg.svd(feats[d], full_matrices=False)
        vt_cache[d] = Vt
    ...
    while len(selected) < k:
        ...
        _, s_cur, Vt_cur = np.linalg.svd(H_cur, full_matrices=False)
        nA = float(s_cur.sum())
        ...
        for d in remaining:
            ...
            pred = cpred(r_cur, rc[d], snorm_cache[d] / (nA + 1e-8), alpha)
            if pred > best_pred:
